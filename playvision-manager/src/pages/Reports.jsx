...
export default function Reports() {
  ...
  const filterClientId = location.state?.clientId ?? null
  ...
  }, [sessions, startDateFilter, endDateFilter, nameFilter, reportType, filterClientId])
  ...
  const machinesTotalPages = Math.max(1, Math.ceil(machinesSummary.length / perPage))
  const pagedMachinesSummary = machinesSummary.slice((page - 1) * perPage, page * perPage)

  const clientsTotalPages = Math.max(1, Math.ceil(clientsSummary.length / perPage))
  const pagedClientsSummary = clientsSummary.slice((page - 1) * perPage, page * perPage)
  ...
  function exportSalesCsv() {
    ...
    downloadCsv(`vendas_${new Date().toISOString().slice(0, 10)}.csv`, csv)
  }

  function exportMachinesCsv() {
    const header = ['Máquina', 'Sessões', 'Tempo jogado', 'Tempo pago', 'Valor']
    const rows = machinesSummary.map(m => [m.machineName, m.sessions.length, formatMinutes(m.totalPlayedMinutes), formatMinutes(m.totalPaidMinutes), formatCurrency(m.totalPaidAmount)])
    const csv = [header, ...rows].map(row => row.map(value => `"${String(value).replace(/"/g, '""')}"`).join(',')).join('\n')
    downloadCsv(`maquinas_${new Date().toISOString().slice(0, 10)}.csv`, csv)
  }

  function exportClientsCsv() {
    const header = ['Cliente', 'Sessões', 'Tempo jogado', 'Tempo pago', 'Valor']
    const rows = clientsSummary.map(c => [c.clientName, c.sessions.length, formatMinutes(c.totalPlayedMinutes), formatMinutes(c.totalPaidMinutes), formatCurrency(c.totalPaidAmount)])
    const csv = [header, ...rows].map(row => row.map(value => `"${String(value).replace(/"/g, '""')}"`).join(',')).join('\n')
    downloadCsv(`clientes_${new Date().toISOString().slice(0, 10)}.csv`, csv)
  }

  function exportDetailedCsv() {
    const header = ['Início', 'Cliente', 'Máquina', 'Tempo', 'Horas pagas', 'Valor', 'Status']
    const rows = pagedSessions.map(s => [new Date(s.started_at).toLocaleString('pt-BR'), s.client_name || 'Cliente desconhecido', s.machine_description || 'Máquina desconhecida', formatMinutes(s.played_minutes), formatMinutes(s.paid_minutes), formatCurrency(s.paid_amount), s.status === 'encerrada' ? 'Encerrada' : 'Em andamento'])
    const csv = [header, ...rows].map(row => row.map(value => `"${String(value).replace(/"/g, '""')}"`).join(',')).join('\n')
    downloadCsv(`historico_${new Date().toISOString().slice(0, 10)}.csv`, csv)
  }

  const reportHeader = useMemo(() => {
    const totalSessions = clientsSummary.reduce((sum, item) => sum + item.sessions.length, 0)
    const totalPlayed = clientsSummary.reduce((sum, item) => sum + item.totalPlayedMinutes, 0)
    const totalAmount = clientsSummary.reduce((sum, item) => sum + item.totalPaidAmount, 0)
    return `${totalSessions} sessoes • ${formatMinutes(totalPlayed)} jogados • ${formatCurrency(totalAmount)}`
  }, [clientsSummary])
        ...
        <article style={{ borderRadius: 18, border: '1px solid rgba(98,227,202,0.3)', padding: 16, background: 'rgba(9,12,25,0.88)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div>
              ...
              <div style={{ color: '#a0a0c8', fontSize: '0.9rem' }}>{machinesSummary.length} máquina(s)</div>
            </div>
            <button className="neon-button" onClick={exportMachinesCsv}>Exportar CSV</button>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <div>
              <strong>Resumo</strong>
              <div style={{ color: '#a0a0c8', fontSize: '0.9rem' }}>
                {machinesSummary.reduce((sum, m) => sum + m.sessions.length, 0)} sessões • {formatMinutes(machinesSummary.reduce((sum, m) => sum + m.totalPlayedMinutes, 0))} jogados • {formatCurrency(machinesSummary.reduce((sum, m) => sum + m.totalPaidAmount, 0))}
              </div>
            </div>
          </div>

          {loading ? (
            <p style={{ color: '#a0a0c8' }}>Carregando...</p>
          ) : machinesSummary.length === 0 ? (
            <p style={{ color: '#a0a0c8' }}>Nenhum registro.</p>
          ) : (
            <div>
              {pagedMachinesSummary.map(m => (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button className="neon-button" onClick={() => setPage(p => Math.min(machinesTotalPages, p + 1))} disabled={page >= machinesTotalPages}>Próxima</button>
                </div>
                <div style={{ color: '#a0a0c8' }}>Página {page} / {machinesTotalPages}</div>
              </div>
            </div>
          )}
        </article>
        ...
        <article style={{ borderRadius: 18, border: '1px solid rgba(98,227,202,0.3)', padding: 16, background: 'rgba(9,12,25,0.88)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div>
              ...
              <div style={{ color: '#a0a0c8', fontSize: '0.9rem' }}>{clientsSummary.length} clientes</div>
            </div>
            <button className="neon-button" onClick={exportClientsCsv}>Exportar CSV</button>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <div>
              <strong>Resumo</strong>
              <div style={{ color: '#a0a0c8', fontSize: '0.9rem' }}>{reportHeader}</div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="neon-button" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}>Anterior</button>
                  <button className="neon-button" onClick={() => setPage(p => Math.min(clientsTotalPages, p + 1))} disabled={page >= clientsTotalPages}>Próxima</button>
                </div>
                <div style={{ color: '#a0a0c8' }}>Página {page} / {clientsTotalPages}</div>
              </div>
            </div>
          )}
        </article>
        ...
        <article style={{ borderRadius: 18, border: '1px solid rgba(255,180,120,0.2)', padding: 16, background: 'rgba(9,12,25,0.88)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div>
              ...
              <div style={{ color: '#a0a0c8', fontSize: '0.9rem' }}>{filteredSales.length} registros</div>
            </div>
          ...
          {loading ? (
            ...
            <p style={{ color: '#a0a0c8' }}>Nenhuma venda encontrada.</p>
          ) : (
        ...
        <article style={{ borderRadius: 18, border: '1px solid rgba(98,227,202,0.3)', padding: 16, background: 'rgba(9,12,25,0.88)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div>
              ...
              <div style={{ color: '#a0a0c8', fontSize: '0.9rem' }}>{filteredSessions.length} registros</div>
            </div>
            <button className="neon-button" onClick={exportDetailedCsv}>Exportar CSV</button>
          </div>
